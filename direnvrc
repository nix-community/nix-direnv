...
_nix_direnv_install_reload() {
  local layout_dir=$1 reload tmp desired existing
  reload="${layout_dir}/bin/nix-direnv-reload"
  desired=$(_nix_direnv_reload_script)

  if [[ -O $reload && -x $reload ]]; then
    existing=$(<"$reload")
  ...
  tmp=$(mktemp "${layout_dir}/bin/.nix-direnv-reload.XXXXXX") || return

  # Write to a temporary file first so the existing helper stays usable if
  # writing the new script or marking it executable fails.
  if ! printf "%s\n" "$desired" >"$tmp"; then
    rm -f "$tmp"
    return 1
  fi
...
_nix_direnv_preflight() {
  ...
  if ! _nix_direnv_install_reload "$layout_dir"; then
    return 1
  fi
