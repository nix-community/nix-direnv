...
`clean` subcommand, which prevents direnv caches from being garbage collected.
This must be explicitly invoked, but prevents garbage collection. Because of
this functionality, we have no longer provide explicit support for marking
gcroots as "active".
